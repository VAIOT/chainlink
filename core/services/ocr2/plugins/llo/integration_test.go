...
func TestIntegration_LLO(t *testing.T) {
	...
	appBootstrap, bootstrapPeerID, _, bootstrapKb, observedLogs := setupNode(t, bootstrapNodePort, "bootstrap_mercury", backend, bootstrapCSAKey)
	...
	t.Run("receives at least one report per feed from each oracle when EAs are at 100% reliability", func(t *testing.T) {
		...
		seen := make(map[ocr2types.Account]map[types.ChannelID]struct{})

		for channelID, defn := range channelDefinitions {
			t.Logf("Expect report for channel ID %x (definiton: %#v)", channelID, defn)
		}
		for _, o := range oracles {
			t.Logf("Expect report from oracle %s", o.OracleIdentity.TransmitAccount)
			seen[o.OracleIdentity.TransmitAccount] = make(map[types.ChannelID]struct{})
		}
		...
		for req := range reqs {
			...
			// TODO: three transmits, one for each channel?
			t.Logf("Got report from oracle %x with format: %s", req.pk, req.req.ReportFormat)

			var r datastreamsllo.Report

			switch req.req.ReportFormat {
			case "json":
				t.Logf("Got JSON report from oracle %x: %s", req.pk, string(report.([]byte)))
				var err error
				r, err = (datastreamsllo.JSONReportCodec{}).Decode(report.([]byte))
				require.NoError(t, err, "expected valid JSON")
			case "evm":
				t.Logf("Got EVM report from oracle %x: 0x%x", req.pk, report.([]byte))
				var err error
			...
			assert.GreaterOrEqual(t, r.ValidAfterSeconds, uint32(testStartTimeStamp))
...
func setConfig(t *testing.T, steve *bind.TransactOpts, backend *backends.SimulatedBackend, verifierContract *channel_verifier.ChannelVerifier, verifierAddress common.Address, nodes []Node, oracles []confighelper.OracleIdentityExtra) ocr2types.ConfigDigest {
	...
	signers, _, _, onchainConfig, offchainConfigVersion, offchainConfig, err := ocr3confighelper.ContractSetConfigArgsForTests(
	...
	verifierContract.SetConfig(steve, signerAddresses, offchainTransmitters, fNodes, offchainConfig, offchainConfigVersion, offchainConfig, nil)
...
func setChannelDefinitions(t *testing.T, steve *bind.TransactOpts, backend *backends.SimulatedBackend, configStoreContract *channel_config_store.ChannelConfigStore, streams []Stream) map[types.ChannelID]channel_config_store.IChannelConfigStoreChannelDefinition {
	channels := []types.ChannelID{
		types.ChannelID(rand.Uint32()),
		types.ChannelID(rand.Uint32()),
		types.ChannelID(rand.Uint32()),
		types.ChannelID(rand.Uint32()),
	}
	...
	configStoreContract.AddChannel(steve, channels[0], channel0Def)
	configStoreContract.AddChannel(steve, channels[1], channel1Def)
	configStoreContract.AddChannel(steve, channels[2], channel2Def)
	configStoreContract.AddChannel(steve, channels[3], channel3Def)

	backend.Commit()

	channelDefinitions := make(map[types.ChannelID]channel_config_store.IChannelConfigStoreChannelDefinition)
