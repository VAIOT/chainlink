...
contract FunctionsClientRequestSetup is FunctionsSubscriptionSetup {
  ...
  struct Request {
    ...
    FunctionsResponse.CommitmentWithOperationFee commitmentWithOperationFee;
  }
  ...
  function _sendAndStoreRequest(
    ...
    s_requests[requestNumberKey] = Request({
      ...
      commitmentWithOperationFee: commitment
    });
    s_requestsSent += 1;
  }
  ...
  function _buildReport(
    ...
    for (uint256 i = 0; i < requestNumberKeys.length; ++i) {
      ...
      _errors[i] = errors[i];
      _onchainMetadata[i] = abi.encode(s_requests[requestNumberKeys[i]].commitmentWithOperationFee);
